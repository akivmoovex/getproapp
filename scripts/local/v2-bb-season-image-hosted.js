...
/**
 * V2.0 BUG 11 — BlessBoard This Season at Church image editing hosted QA.
...
const EXPECTED_SHA_PREFIX = process.env.V2_BB_SEASON_IMAGE_EXPECTED_SHA || "PLACEHOLDER";
const OUT = process.env.V2_BB_SEASON_IMAGE_OUT || "/tmp/v2-bb-season-image-hosted.json";
...
const DEMO_TITLES = {
  "demo-event-1": "Leaders Equipping Weekend",
  "demo-event-2": "Sunday Morning Connection",
  "demo-event-3": "Neighbourhood Celebration",
};
...
function req(jar, url, opt = {}) {
  return new Promise((resolve, reject) => {
    ...
    let body = null;
    const headers = {
      "User-Agent": "Mozilla/5.0 V2SeasonImageQA",
      Cookie: jar.header(),
...
async function uploadPng(jar, uploadUrl, token, filename) {
  const boundary = `----V2SeasonImg${Date.now()}`;
...
async function postEventDraft(jar, token, entityKey, payload) {
...
async function main() {
  ...
  if (EXPECTED_SHA_PREFIX === "PLACEHOLDER") {
    result.blockReason = "set_V2_BB_SEASON_IMAGE_EXPECTED_SHA";
  ...
  result.editChrome = {
    ...
    hasThisSeason: /This Season at Church/i.test(editPage.body),
    hasEventsSection: /data-bb-home-events="1"/.test(editPage.body),
    editImageCount: triggers.length,
    entityKeys: triggers,
    distinctKeys: new Set(triggers).size,
    structuredBust: /website-structured-edit\.js\?v=v2-bb-season-img-1/.test(editPage.body),
    uploadUrl: uploadUrl || null,
  };

  if (triggers.length < 2 || new Set(triggers).size < 2) {
  ...
  const uploads = [];
  const saves = [];
  const targetKeys = triggers.slice(0, 2);
  ...
  for (let i = 0; i < targetKeys.length; i += 1) {
    ...
    const uploaded = await uploadPng(auth.jar, upUrl, token, `season-${stamp}-${i}.png`);
    ...
    const title = DEMO_TITLES[entityKey] || `Season Event ${i + 1}`;
    const startsAt = new Date(Date.now() + (i + 2) * 86400000).toISOString();
    const save = await postEventDraft(auth.jar, token, entityKey, {
      title,
      summary: `V2 BUG11 season image QA ${stamp} ${i}`,
      startsAt,
      endsAt: null,
      timezone: "UTC",
  ...
  // Grow and Serve (Bug 10) still exposes Edit image — same shared trigger path.
  result.bug10Regression = {
    ministryEditImages: (draftPage.body.match(/data-bb-home-ministries[\s\S]*?aria-label="Edit image"/) || [])
      .length
      ? (draftPage.body.match(/data-bb-kind="ministry"[\s\S]{0,200}?aria-label="Edit image"/g) || []).length
      : (draftPage.body.match(/data-bb-home-ministry-cards[\s\S]*?aria-label="Edit image"/g) || []).length,
    ok: /data-bb-home-ministry-cards="1"/.test(draftPage.body) && /aria-label="Edit image"/.test(draftPage.body),
  };
  ...
  result.status =
    result.editChrome.hasThisSeason &&
    result.editChrome.editImageCount >= 2 &&
    result.editChrome.distinctKeys >= 2 &&
    result.editChrome.structuredBust &&
    uploads.every((u) => u.ok) &&
    saves.every((s) => s.ok) &&
    result.draftPreview.ok &&
    result.isolation.ok &&
    result.bug10Regression.ok &&
    result.publish.ok &&
  ...
  if (result.status !== "PASS") {
    result.blockReason = [
      !result.editChrome.hasThisSeason && "missing_this_season_heading",
      result.editChrome.editImageCount < 2 && "missing_edit_image_triggers",
      result.editChrome.distinctKeys < 2 && "non_independent_entity_keys",
      ...
      !result.bug10Regression.ok && "bug10_ministry_regression",
