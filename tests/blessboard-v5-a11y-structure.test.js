  ...
  for (const shell of shells) {
    it(`${shell.name} shell has skip link, main landmark, and drawer wiring`, () => {
      ...
      if (shell.name === "hq") {
        ...
        assert.match(start, /Broadcasts/i);
        assert.match(start, /\/hq\/broadcasts/);
...
describe("blessboard v5 a11y structure — shell-nav + media picker", () => {
  ...
  it("HQ branch registry keeps searchable list without fabricated metrics", () => {
    ...
    assert.match(createPage, /data-bb-hq-branch-new-v2="1"/);
    assert.match(createPage, /data-bb-branch-step="identity"/);
    assert.match(createPage, /data-bb-branch-step="location"/);
    assert.match(createPage, /data-bb-branch-step="review"/);
    assert.match(createPage, /data-bb-branch-key-regen="1"/);
    assert.match(createPage, /data-bb-branch-url-preview="1"/);
    assert.match(createPage, /branchKeyManuallyEdited/);
    assert.match(createPage, /\/c\/.*\/branches\//);
  ...
  it("platform admin dashboard keeps Stitch sections without fabricated metrics", () => {
    ...
    assert.match(paShell, /platform-admin.css\?v=\d+/);
    ...
    assert.match(dash, /\/admin\/system\/deployments|\/admin\/deployments/);
  ...
  it("platform admin settings keeps Stitch DNS patterns without automation or save controls", () => {
    ...
    assert.match(page, /data-bb-pa-settings-deploy(?:ment)?(?:-link)?="1"|data-bb-pa-settings-deployment="1"/);
    ...
    assert.match(page, /data-bb-pa-unavailable="org-create-form"|data-bb-pa-unavailable="deploy-create-form"/);
    ...
    assert.match(page, /currentDeployment|links\.deployments|Deployment registry/);
    assert.match(page, /links\.deployments|\/admin\/(?:system\/)?deployments/);
  ...
  it("platform admin deployments directory keeps Stitch layout without ops or secrets", () => {
    ...
    assert.match(page, /href="\/admin\/(?:system\/)?deployments\/<%= encodeURIComponent\(d\.deploymentCode\) %>"/);
  ...
  it("platform admin deployment detail keeps safe diagnostics without secrets or ops", () => {
    ...
    assert.match(page, /href="\/admin\/(?:system\/)?deployments"/);
    assert.doesNotMatch(page, /method="post"/);
    assert.doesNotMatch(page, /session_cookie|SESSION_SECRET|DATABASE_URL|credential/i);
    assert.doesNotMatch(page, /Force Sync|Export Reports|Support Tickets|99\.98%|Critical Error Rate/i);
    ...
    assert.match(routes, /\/admin\/(?:system\/)?deployments\/:deploymentCode/);
  ...
  it("branch admin settings keeps editable V5 fields and unavailable HQ/product sections", () => {
    ...
    assert.match(settings, /gp-location-field|fieldName:\s*'city'|name="city"/);
    assert.match(settings, /name="provinceState"|provinceState/);
...
describe("blessboard v5 a11y structure — viewport CSS breakpoints present", () => {
  ...
  it("tenant auth templates preserve CSRF fields and omit unsupported auth chrome", () => {
    ...
    assert.match(login, /name="login_email"|name="email"/);
    assert.match(login, /name="password"/);
    assert.match(login, /name="phone_national"|phone_national/);
    assert.match(login, /data-gp-auth-identifier="1"|data-bb-stitch-login="09-auth-member-login"/);
    ...
    assert.doesNotMatch(login, /name="next"|name="return_to"/i);
    assert.match(login, /Forgot password/i);
    assert.match(login, /data-bb-auth-forgot="1"/);
