...
test("branch admin shell references church.css?v=42", () => {
...
test("public and member shells still on v41", () => {
  for (const rel of [
    ...
    assert.match(text, /church\.css\?v=42/, `${rel} should use v41`);
  }
});
  ...
  async () => {
    ...
    try {
      ...
      for (const screen of screens) {
        ...
        assert.match(res.text, /church\.css\?v=42/, `${screen.path} CSS v41`);
