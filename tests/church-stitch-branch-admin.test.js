...
test("branch admin shell references church.css?v=45", () => {
  const text = fs.readFileSync(
    path.join(__dirname, "../views/church/partials/branch_admin_shell_start.ejs"),
    "utf8"
  );
  assert.match(text, /church\.css\?v=45/);
...
test("public and member shells still on v41", () => {
  for (const rel of [
    ...
    assert.match(text, /church\.css\?v=45/, `${rel} should use v41`);
  }
});
  ...
  async () => {
    ...
    try {
      ...
      for (const screen of screens) {
        ...
        assert.match(res.text, /church\.css\?v=45/, `${screen.path} CSS v43`);
