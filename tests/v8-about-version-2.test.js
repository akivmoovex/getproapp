...
/**
 * V8-BUG-001 — About pages show Version 2.0 + real Git build on V8;
...
describe("V8-BUG-001 About Version 2.0", () => {
  it("keeps shared V7 constants and resolves V8 scheme from deployment code", () => {
    ...
    assert.equal(VERSION_BASE_V8, "2.0");
    assert.equal(PRODUCT_VERSION_V8, "2.0");

    const v8 = resolveVersionScheme(V8_ENV_BASE);
    const v7 = resolveVersionScheme(V7_ENV_BASE);
    assert.equal(v8.platformLine, "v8");
    assert.equal(v8.productVersion, "2.0");
    assert.equal(v7.platformLine, "v7");
    assert.equal(v7.productVersion, "1.3");
  });

  it("formats V8 About metadata as Version 2.0 with separate Git build (shared for BB/AC)", () => {
    ...
    assert.equal(info.productVersion, "2.0");
    assert.equal(info.versionBase, "2.0");
    assert.equal(info.version, "2.0");
    assert.equal(info.productVersionLabel, "Version 2.0");
    assert.equal(info.build, sha.slice(0, 12));
    assert.equal(info.available, true);
    assert.notEqual(info.version, `2.0.${info.build}`);
  ...
  it("BlessBoard V8 GET /about shows Version 2.0 and the real Git build", async () => {
    ...
    try {
      ...
      assert.match(res.text, /Release 2\.0/);
      ...
      assert.doesNotMatch(res.text, /2\.0\.abcdef012345/);
    } finally {
      app.__restoreEnv();
    }
  });

  it("ActiveClinic V8 GET /about shows Version 2.0 and the real Git build", async () => {
    const {
    ...
    assert.match(res.text, /Enterprise v2\.0/);
    assert.match(res.text, new RegExp(`>${sha.slice(0, 12)}<`));
    assert.match(res.text, /href="\/about"/);
    assert.match(res.text, /Version 2\.0/);
    assert.doesNotMatch(res.text, /1\.03\./);
    assert.doesNotMatch(res.text, /Enterprise v1\.3/);
    assert.doesNotMatch(res.text, /2\.0\.fedcba987654/);
  });
