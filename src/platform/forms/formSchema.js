...
/**
 * Shared Moovex form schema validation (BlessBoard + ActiveClinic).
 * Allowlisted field types and declarative rules only —
 * no executable / HTML / script / clinical-intake schemas.
 */
...
/** Categories allowed on shared tenant forms (clinical intake excluded). */
const ALLOWED_FORM_CATEGORIES = Object.freeze([
  "general",
  "feedback",
  "registration",
  "event",
...
/** Declarative validation rule kinds only — never arbitrary code. */
const ALLOWED_VALIDATION_RULE_KINDS = Object.freeze([
  "required",
  "min",
  "max",
  "maxLength",
...
function rejectsExecutableValidation(parsed) {
  if (!parsed || typeof parsed !== "object") return false;
  const bannedTop = ["validationRules", "validators", "rules", "onSubmit", "onValidate"];
  for (const key of bannedTop) {
    if (Object.prototype.hasOwnProperty.call(parsed, key)) {
      ...
      if (Array.isArray(value)) {
        for (const rule of value) {
          ...
          const kind = String(rule.kind || rule.type || "").trim();
          if (!ALLOWED_VALIDATION_RULE_KINDS.includes(kind)) return true;
          if (
  ...
  if (Array.isArray(parsed.fields)) {
    for (const field of parsed.fields) {
      ...
      if (field.validation != null || field.validator != null || field.validate != null) {
        const v = field.validation || field.validator || field.validate;
        if (typeof v === "string" || typeof v === "function") return true;
        if (v && typeof v === "object" && !Array.isArray(v)) {
          if (v.expression || v.code || v.script || v.pattern || v.regex) return true;
        }
      }
    }
  }
 ...
 * @returns {{ ok: true, category: string } | { ok: false, reason: string }}
 */
function validateFormCategory(category) {
  const raw = String(category == null || category === "" ? "general" : category)
    .trim()
    .toLowerCase();
  if (FORBIDDEN_FORM_CATEGORIES.includes(raw)) {
    return { ok: false, reason: "category_clinical_excluded" };
  }
  if (!ALLOWED_FORM_CATEGORIES.includes(raw)) {
...
function validateFormSchema(raw) {
  ...
  if (rejectsExecutableValidation(parsed)) {
    return { ok: false, reason: "executable_validation_not_allowed" };
  }
  ...
  for (const field of parsed.fields) {
    ...
    if (rejectsExecutableValidation({ fields: [field] })) {
      return { ok: false, reason: "executable_validation_not_allowed" };
    }
...
module.exports = {
  ...
  FORBIDDEN_FORM_CATEGORIES,
  ALLOWED_VALIDATION_RULE_KINDS,
  LIMITS,
