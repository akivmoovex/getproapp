 ...
 * (blessboard-com-production, blessboard-org-staging, and deprecated aliases).
 */
...
const SKIPPED_LEGACY_MODULES = Object.freeze([
  ...
  "in-process job workers (cron scripts still respect profile jobsEnabled)",
...
/**
 * True when an authoritative production BlessBoard profile is active
 * (V5 foundation on blessboard.com / production database expectation).
 * @param {NodeJS.ProcessEnv} [env]
 */
function isBlessBoardProductionRuntime(env) {
  ...
  return Boolean(
    profile &&
      ...
      profile.deploymentEnvironment === "production"
  );
}
...
function logV5FoundationModeActive(logFn) {
  ...
  const depEnv = (profile && profile.deploymentEnvironment) || "testing";
  out("[blessboard] V5 foundation mode: ACTIVE");
  out(
    `[blessboard] V5 foundation mode: PLATFORM_DEPLOYMENT_CODE=${code} (deployment environment derived as ${depEnv})`
  );
