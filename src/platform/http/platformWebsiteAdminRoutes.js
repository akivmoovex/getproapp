...
function registerPlatformWebsiteAdminRoutes(router, deps) {
  ...
  router.post("/admin/clinic-registrations/:applicationId/approve", requireApex, requirePlatformAdmin, async (req, res, next) => {
    try {
      ...
      if (!result.ok) {
        return res.redirect(303, `/admin/clinic-registrations?error=${encodeURIComponent(result.code)}`);
      }
      return res.redirect(303, `/admin/clinic-registrations?notice=approved`);
    } catch (err) {
      return next(err);
    }
  });
