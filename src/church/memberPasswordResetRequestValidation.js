...
const PUBLIC_SUCCESS_MESSAGE =
  "If your details match a member account, the church office will review your request.";
...
function validatePublicForgotPasswordBody(body) {
  ...
  if (!identifier) {
    return { ok: false, error: "Please enter your email or phone number.", form: { full_name, phone: phoneRaw, email: emailRaw } };
  }

  const email = emailRaw ? normalizeEmail(emailRaw) : "";
  if (emailRaw && (!email || !email.includes("@"))) {
    return { ok: false, error: "Please enter a valid email address.", form: { identifier, full_name, phone: phoneRaw, email: emailRaw } };
  }
...
function validateBranchAdminResetPasswordBody(body) {
