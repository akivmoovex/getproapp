...
| `BLESSBOARD_INSTANT_FREE_PROVISIONING_ENABLED` | Unset = **on** (default). Set `0`/`false`/`no`/`off` to disable. | Disable: `0`\|`false`\|`no`\|`off`. Enable: unset\|`1`\|`true`\|`yes`\|`on`. Unsupported → **disabled** (fail-closed). | No | Automatic Foundation (`foundation` → DB `free`) and Growth (one-month `trialing`) self-service provision on `/register-church`. Network always enquiry/support-contact only. | Unset → **enabled**. Explicit false disables (enquiry-only emergency switch). Query/body cannot override. See `docs/FOUNDATION_INSTANT_FREE_REGISTRATION_IMPLEMENTATION.md`. |
