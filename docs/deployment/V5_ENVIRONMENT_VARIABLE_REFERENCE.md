...
| `BLESSBOARD_INSTANT_FREE_PROVISIONING_ENABLED` | Recommended unset/`0` until ops smoke-test | Disable: unset\|`0`\|`false`\|`no`\|`off`. Enable: `1`\|`true`\|`yes`\|`on`. | No | Instant Free self-service provision on `/register-church` via `provisionRegisteredBlessBoardChurch` | Unset/invalid → **disabled** (enquiry-only). Growth/Network never instant. Query/body cannot override. See `docs/FOUNDATION_INSTANT_FREE_REGISTRATION_IMPLEMENTATION.md`. **Do not enable for production users before manual verification.** |
