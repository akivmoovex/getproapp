...
| 6a | Apex marketing | Church Registration Successful (BBR08) | `6d37fb8a90ce466b9a5a9457159b36aa` | `f9a850ec020942d887705b8b56921db1` | `/register-church/success` | `/register-church/success` | `apex/register-church-success.ejs`, `apex/register-church-success-panel.ejs` | anon | PARTIAL | Screens live in Stitch project `5480028321322186480` (BlessBoard User Management). Desktop canvas 1280×1290; mobile 390×948. Desktop follows BBR08-01-D. Mobile follows BBR08-01-M composition (Church ID label, confirmation/next-step cards, receipt print). Live value remains sanitized `BB-…`. Confirmation body does not claim a registration email was sent (that mailer is not wired). |
...
| 37 | Branch admin | Dashboard | `001d1a0235a14f47b456bb092a012f7c` | `615f1f4eabd645c4a6840349edb17cd1` | `/branch-admin` | `/branch-admin` | `branch-admin/dashboard.ejs`, `branch-admin-shell-*` | branch_admin | PARTIAL | Prefer 25-* over obsolete 04-* duplicates. Batch 11B: Stitch layout with unavailable summary cards; no fabricated counts/budget/activity. |
| 38 | Branch admin | Verification queue | `87fe9bb70b79434e88b91e0fd877d238` | `d352ed076bbe4fabb1ad6f5ef66c0a25` | `/branch-admin/registrations` | `/branch-admin/registrations` | `branch-admin/registrations.ejs` (+ `registration-detail.ejs`) | branch_admin | PARTIAL | Batch 12A queue + Batch 12B detail. No dedicated detail Stitch pair — identity/review chrome adapted from 26-* (+ member-profile layout cues). |
...
| 52 | Branch admin | Giving summary | `cf849cdb676c48fd8f0f7d38b74c99b0` | `20f32c9e77af423ca6a849a6759add28` | `/branch-admin/giving` (+ `/new`, `/:id`) | `/branch-admin/giving` | `giving/admin-list.ejs` (+ `admin-form.ejs`, `admin-detail.ejs`) | branch_admin | PARTIAL | Batch 15B. Titles: `39-branch-giving-summary-*`. Aggregate monthly cards + history table/cards + manual entry. Existing fields only; no donor PII, payments, receipts, exports, or fabricated trends. |
| 52a | Branch admin | Forms management | — | — | `/branch-admin/forms` (+ `/:id`) | `/branch-admin/forms` | `forms-requests/admin-forms.ejs` (+ `admin-form-detail.ejs`) | branch_admin | PARTIAL | Batch 15C (+ HQ Batch 18D shared template). **STITCH_MISSING** dedicated pair. Shared UI States + announcements-management chrome. Allowlisted schema editor + branch-scoped submissions. No signatures/payments/conditional logic. HQ uses member forms-documents 20. |
| 53 | Branch admin | Reports dashboard | `d7bdddc0eb244b96a35d6aa6b9bb8f97` | `cc7a0658d477494e9a47c4852b7ed84b` | — | — | — | branch_admin | MISSING | V4 monthly reports not ported to V5 branch UI. |
| 54 | Branch admin | Submit monthly report | `45a88626123b4fc8892ac50244dd561b` | `1be0cd7ef8f8433bbd44c6d1573e495c` | — | — | — | branch_admin | MISSING | |
| 55 | Branch admin | Report history | `5b6ec354f47c4ceda67739b07136a6c0` | `28a3a26278fb4b639b7ffe7bbf408bb1` | — | — | — | branch_admin | MISSING | |
| 56 | Branch admin | Report details | `48955e5ac03442acb363d8d0dd68975e` | `4767ec0f410a4287ba79bdcf049a7cc0` | — | — | — | branch_admin | MISSING | |
| 57 | Branch admin | Request queue | `126bfebff1414fc08367039b84587819` | `9b6531097eec43fb8ce22115dd170429` | `/branch-admin/requests` | `/branch-admin/requests` | `forms-requests/admin-requests.ejs` | branch_admin | PARTIAL | Batch 15D. Titles: `44-branch-request-workflow-queue-*`. Real status tabs + desktop table/mobile cards; truncated memberRef. No Pending/Goal KPIs, assignment, Export, Urgent, or donor/contact PII. |
| 58 | Branch admin | Request details | `22fe4b70e55e4be498d7008741147d55` | `9d8f71d056e54d7da7586d88e253af93` | `/branch-admin/requests/:id` | `/branch-admin/requests/:id` | `forms-requests/admin-request-detail.ejs` | branch_admin | PARTIAL | Batch 15D. Titles: `45-branch-request-details-*`. Status transitions + history + private attachment download preserved. No chat, SLA, Approve/Reject chrome, or member profile cards. |
| 59 | Branch admin | Account | — | — | `/branch-admin/account` | `/branch-admin/account` | `branch-admin/account.ejs` | branch_admin | STITCH_MISSING | Batch 11C identity summary + role badge + church/branch context; no dedicated Stitch pair. Session-safe; logout CSRF preserved. |
...
| 64 | HQ admin | Consolidated analytics | `2a577dc15d4342acb152f16aed21c267` | `06489c79d0d04a429e57eba5c717ba47` | `/hq/reports`, `/hq/reports/attendance`, `/hq/reports/giving`, `/hq/reports/executive` | same | `hq/reports.ejs`, `hq/attendance-report.ejs`, `hq/giving-report.ejs`, `hq/executive-dashboard.ejs` | hq_admin | PARTIAL | Batch 18B/18C + NW-EX-01 executive (Network `executive_reports`). Real monthly aggregates + accessible bar tables; no donor PII, trends, forecasts, or canvas charts. |
...
| 66 | HQ admin | Global audit trail | `bce1e8ec4078407c8d6179251b8765c2` | `d7fcb1b3a796434a8fefc7e806c2c0b6` | `/hq/audit`, `/hq/audit/governance` | same | `hq/audit.ejs`, `hq/governance-audit.ejs` | hq_admin | PARTIAL | Batch 18F basic trail + NW-GOV-01 Network governance filters (`advanced_audit`). Append-only; truncated refs; no metadata/secrets/CSV/compliance scores. |
...
| 71b | HQ admin | Forms oversight | `745a1972c0ba4ec893f64cc3457c0c95` | `0f801e19ed3d4332bee877001bdc1a13` | `/hq/forms` (+ `/b/:branchKey`) | `/hq/forms` | `forms-requests/admin-forms.ejs` (+ detail) | hq_admin | PARTIAL | Batch 18D (`BATCH_18D_HQ_FORMS_RESOURCES.md`). Reuses member forms-documents 20. Branch panel + status/search; list omits submission answers; no builder/signatures/payments/export dumps. |
| 71c | HQ admin | Resources oversight | `d1690ab7193d43e38ba9ba97c29d914c` | `d3232a4f5e0f4d2da610740ca3a8f6b1` | `/hq/resources` (+ `/b/:branchKey`) | `/hq/resources` | `forms-requests/admin-resources.ejs` | hq_admin | PARTIAL | Batch 18D. Reuses member resources-study 19. Branch panel + status/search; private-file label only; no progress/certificates/public CDN. |
...
| 86 | Leader portal (out of V5 scope) | Submit ministry report | `202f402c811b4975a0da4c39c1e55d22` | `b4ecb0d74f58402ea20f84383b84e821` | — | — | — | leader | MISSING | |
