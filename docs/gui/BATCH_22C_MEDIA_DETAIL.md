...
**Date:** 2026-07-18  
**Scope:** Presentation for **asset detail** (side panel / mobile stack) and existing **soft-archive confirmation**. No replace, crop, rename, bulk-delete, or hard-delete. No route/schema/storage changes.  
...
| Path | Change |
|------|--------|
| `public/blessboard/v5/media-picker.js` | Detail panel metadata + usage copy; richer archive confirm; inline archive errors |
...
**Unchanged:** church-scoped `UPDATE … AND church_id = $2`, soft-archive semantics (object retained), list DTO fields, storage adapters, schema. Archive CSRF check aligned with upload JSON (`403` + `{ ok:false, reason:"csrf" }`) so the picker confirm dialog can show a safe message.
...
**Usage copy:** Explains soft-archive removes library + public delivery while retaining the stored object for audit. Honest note that reference / dependency counts are **not** reported by this release (no fabricated in-use API).

## 4. Delete / archive safeguards

| Safeguard | Behaviour |
|-----------|-----------|
| Soft-archive only | `POST …/archive` sets `status=archived`; object not deleted |
...
| Command | Result |
|---------|--------|
| `npm run test:blessboard:media` | **21/21 pass** (detail UI, authorized archive, cross-tenant 404, soft-archive honesty, CSRF) |
| `npm run test:blessboard:a11y-structure` | **83/83 pass** (detail/confirm markers, `?v=6`) |
