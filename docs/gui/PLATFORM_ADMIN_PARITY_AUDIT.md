...
Media batches 22A–C are complete separately; final GUI validation is in [`V5_FULL_GUI_REGRESSION_AUDIT.md`](./V5_FULL_GUI_REGRESSION_AUDIT.md).
...
| Organization Directory | `18da9665bc674d2dbd249cbbb269d58d` (63-*) | `db6b741d99e34d10b01496a83de5072a` | **CLOSE PARITY** | Key-prefix search + table/cards; empty/no-results; no create/export/MRR |
...
| Domains directory | `30e3856782bd41b6bf14402e1e535cbd` (67-*) | `efb0fd24f1184968be79083974dcd092` | **CLOSE PARITY** / **BLOCKED BY MISSING STITCH** | Adapted from Settings; live filters + table/cards; Clear on no-results |
| Domain Detail | `30e38567…` (67 adapted) | `efb0fd24…` | **CLOSE PARITY** / **BLOCKED BY MISSING STITCH** | Operational vs verification; CSRF status/org when deployment-scoped; breadcrumb |
...
| Deployment Detail | `74cbe4a0…` (68 adapted) | `9f400420…` | **CLOSE PARITY** / **BLOCKED BY MISSING STITCH** / **BLOCKED BY DATA** | Safe summary/env/products/domains + catalogue diagnostics; breadcrumb |
| Settings | `30e38567…` | `efb0fd24…` | **CLOSE PARITY** | Read-only DNS patterns + reserved labels; breadcrumb; no save UI |
| Account | — | — | **BLOCKED BY MISSING STITCH** | Identity panel + logout CSRF; breadcrumb + `aria-label` |
...
| Fix | Files |
|-----|-------|
| Breadcrumbs on Account, Settings, Domain Detail, Deployment Detail, Organization Detail | already present; verified |
| Account: `aria-label`, identity panel, CSRF field uses shell `csrfField` | `account.ejs` |
| Filtered no-results Clear actions on Subscriptions + Domains (+ Orgs empty partial) | verified |
| Broader `:focus-visible` including deploy card / domain-card / domain-form controls | `platform-admin.css` |
| Touch min-height on `.bb-pa-btn`; filter action touch; muted chip contrast; key wrap; bottom-tab type at 320 | verified |
| Shell-end fallback nav aligned to full `PLATFORM_ADMIN_NAV` | verified |
| CSS cache bump | `platform-admin.css?v=24` |
...
| Keyboard / focus-visible on shell + list/detail controls | Pass (extended this audit for deploy cards) |
| Headings | Pass (`h1` + section titles) |
| Breadcrumbs on detail / account / settings | Pass |
| Form labels + CSRF | Pass on POST surfaces (plan assign, entitlement override, domain status/org, logout) |
| Contrast (violet on warm surfaces; muted chips) | Pass within Sacred Modernity tokens |
| Empty / no-results states | Pass; Clear on filtered subs/domains |
| `prefers-reduced-motion` | Pass |
| Touch ≥44px on buttons / icon controls | Pass |
...
**Out of scope:** Shared media (Batch 22+) — do not start from this audit.
