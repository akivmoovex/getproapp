...
V8 tip labeled product Version **2.01**. Includes About bump, publish diagnostics, Website Change Manager (local QA), Hostinger infra investigations (separate from app features).

### Release date

2026-09-25 (baseline / diagnostics / CM task dates).
...
| F-2.01-CM-FOUND-01 | Change Manager foundation | IMPLEMENTED | LOCAL QA PASS | Hosted deploy not in CM tasks |
| F-2.01-CM-TOOLBAR-01 | Toolbar + unpublished counter | IMPLEMENTED | LOCAL QA PASS | |
| F-2.01-CM-REMIND-01 | Publishing reminders | IMPLEMENTED | LOCAL QA PASS | |
| F-2.01-CM-PANEL-01 | Unpublished Changes Panel | IMPLEMENTED | LOCAL QA PASS | |
| F-2.01-CM-HIST-01 | Field history & restore | IMPLEMENTED | LOCAL QA PASS | |
| F-2.01-CM-STITCH-01 | Stitch ~390px parity (CM screens) | IMPLEMENTED | LOCAL QA PASS | |
...
| F-2.01-RNC-01 | Release Notes Center | IMPLEMENTED | LOCAL QA PASS | Hosted deploy NOT TESTED |
...
- Change Manager hosted deploy + hosted QA.
- Package A + PID consolidation proof.
- Original intermittent publish failure not reproduced.

### QA verification

**Mixed — not a full V2.01 release PASS.** See checklist in Release Notes Center `/release-notes/2.01/qa`.

### Deployment status

TESTING target `moovex-platform-v8-testing`. Production untouched in V2.01 task docs. Individual features vary on whether they were hosted-deployed.

### Acceptance criteria

- About shows 2.01 on BB+AC V8 testing after baseline deploy.
- Publish failures no longer remapped to incorrect `lookup_error`.
- Change Manager features not described as RELEASED without hosted evidence.
