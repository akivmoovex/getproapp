(function () {
  ...
  // Shared platform media field owns upload / Content Library / remove when present.
  var usesSharedMediaField = Boolean(document.querySelector("[data-gp-we-media-field]"));

  if (!usesSharedMediaField) {
      ...
      btn.addEventListener("click", function () {
        ...
        fetch(url, { credentials: "same-origin", headers: { Accept: "application/json" } })
          ...
          .then(function (json) {
            ...
            if (!items.length) {
              grid.innerHTML =
                '<p class="bb-wb-muted">No media yet. Upload an image from your computer or open the Content Library.</p>';
              return;
            }
            grid.innerHTML = "";
  ...
  function mediaSrcValue(field) {
    if (!field) return "";
    var srcInput =
      field.querySelector("[data-gp-we-media-src]") ||
      field.querySelector("[data-bb-wb-media-src]");
  ...
  function syncBrandPreview() {
    ...
    var fields = brandForm.querySelectorAll("[data-bb-wb-media-field], [data-gp-we-media-field]");
    var logoField = fields[0];
    var heroField = fields[1];
    var logoSrc = mediaSrcValue(logoField);
    var heroSrc = mediaSrcValue(heroField);
